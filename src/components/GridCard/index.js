...
import CareArticleApi from '../../../Service/LandingPage/Care/CareArticle'
...
const useStyles = makeStyles(theme => ({
	...
	firsttxt: {
		...
		borderRadius: '28px 25px 0px 0px',
		'& .MuiTypography-h5': {
			color: '#fff',
			fontSize: 16,
			padding: 16,
...
function HealthTips() {
	...
	useEffect(() => {
		...
		CareArticleApi.CareArticle().then(onSuccess, onFailure)
	}, [])
	return (
		<div className={classes.PositionMainroot}>
			...
			<div className={classes.Positionroot}>
				<div className={classes.gfg}>
					<Image
						src={
							singleImg?.thumbnailImageUrl
								? singleImg?.thumbnailImageUrl
								: 'https://ik.imagekit.io/lyfngo/web_b2c/public/images/picture/wellnessheadache.png'
						}
						quality={100}
						alt='articles'
						width={520}
					...
					<div className={classes.secondtxt}>
						<Typography variant='h4' style={{webkitLineClamp: '3'}}>
							{singleImg?.body}
						</Typography>
					</div>
				</div>
					...
					{filteredItems?.map(item => (
						<div key={item?.tentArticlesUuid} className={classes.gfg1}>
							...
							<div className={classes.secondtxt2}>
								<Typography variant='h4' style={{webkitLineClamp: '2'}}>
									{item?.title}
								</Typography>
							</div>
						</div>
