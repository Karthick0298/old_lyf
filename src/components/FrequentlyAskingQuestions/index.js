...
import CareFaqApi from '../../../Service/LandingPage/Care/CareFaq'
...
export default function Faq() {
	...
	useEffect(() => {
		...
		CareFaqApi.CareFaq().then(onSuccess, onFailure)
	}, [])
			...
			<div className={classes.priorityImage}>
				<div className={classes.leftAccord}>
					{list.slice(0, 3)?.map(item => (
						<Accordion
							key={item?.mastFaqHelpUuid}
							expanded={expandedPanel === item?.mastFaqHelpUuid}
							onChange={handleAccordionChange(item?.mastFaqHelpUuid)}>
							<AccordionSummary expandIcon={<ExpandMoreIcon />}>
								<>
									{expandedPanel === item?.mastFaqHelpUuid ? (
